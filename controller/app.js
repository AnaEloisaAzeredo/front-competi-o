...
// Aplica animação escalonada nos cards
function animarCards(containerId) {
    const cards = document.querySelectorAll(`#${containerId} .card`);
    cards.forEach((card, i) => {
        card.style.animation = 'none';
        void card.offsetWidth;
        card.style.animation = `fadeInUp 0.5s ease ${i * 0.06}s backwards`;
    });
}
...
function renderizarDashboard() {
    ...
    animarCards('dashboard-stats');
    animarCards('upcoming-matches');
}
        ...
        return `
            <div class="card">
                ...
                <div style="margin-top: 1rem; text-align: center;">
                    ...
                    ${c.status === 'scheduled'
                        ? `<button onclick="encerrarConfrontos(${c.id})" class="btn-primary" style="padding: 4px 10px; font-size: 0.7rem; margin-left: 8px;">Finalizar</button>`
                        : ''}
                </div>
            </div>
        `;
    }).join('');
    animarCards('list-confrontos');
}
